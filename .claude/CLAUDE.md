...
- Removed the global Elementary/HS-College header toggle and locked non-pipeline views to Dataset A to separate recommendation browsing from pipeline iteration inputs.
- Refactored pipeline backend to derive chronological dataset sources automatically (A, B, then uploads by file creation order), and to build cumulative iterations from that ordered history before drift simulation.
